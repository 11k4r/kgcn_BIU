...
for agg in ['sum', 'concat', 'neighbor']:
    for k in range(1, 7):
        file = open('final_results.csv', 'a', newline='', encoding='utf-8')
        ...
        writer.writerow(['music', agg, 'k', 2**k, auc, f1])
        file.close()
    ...
    for dim in range(2, 8):
        ...
        auc, f1 = train(args, data, show_loss, show_topk)
        tf.reset_default_graph()
        writer.writerow(['music', agg, 'dim', 2**dim, auc, f1])
        file.close()
    ...
    for H in range(1, 5):
        ...
        auc, f1 = train(args, data, show_loss, show_topk)
        tf.reset_default_graph()
        writer.writerow(['music', agg, 'h', H, auc, f1])
        file.close()
